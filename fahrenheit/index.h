...
<div class="container mt-4" >
  ...
  <div class="row">
   ...
   <dialog id="dialogggg">
		<h6>Set Temperature(°F):</h6>
		<form action="/setTemperature" method="get">
	    <input type="number" id="settemp" name="t" min="40" max="80" value="40">
		<input type="submit" value="Set">
		</form>
    <br>
		<form action="/setTemperature" method="get">
	...
	<div class="col-lg-5 col-md-6 col-xs-11 pl-1 pr-1">
  		...
  		<h3>Heater Stats</h3>
  		<h5>Time: <span id="TIMEValue"></span>&nbsp;&nbsp;Temp: <span id="TEMPFValue"></span>&deg;F&nbsp;Bat: <span id="BATValue"></span>V&nbsp;&nbsp;Hum: <span id="HUMValue"></span>%</h5>
		<span id="MOREValue" style="color:red;"></span>
		<select name="history" id="history">
			<option value="history">History</option>
		</select>
		<p>polling every <input type="number" id="intv" value="30" min="10"/> seconds</p>
  	</div>
  	</div>
...
function getData() {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (isnum) {
       document.getElementById("MOREValue").innerHTML = "<h4 class=\"animate-flicker\">"+respon[4]+"&deg;F target set</h4>";
      }
       else if (respon[4] == " ") {
